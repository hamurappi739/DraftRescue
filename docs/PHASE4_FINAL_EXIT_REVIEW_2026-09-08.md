...
- `PHASE4-DPAPI-RUNTIME-PROBE.json` is schema v3. The compiled probe checks both the payload and installation-secret DPAPI paths and completed with `probeExitCode=6` (`DpapiFailure`) and `harnessExitCode=0`, so this is a repeatable target-environment result rather than a harness failure; `failureStage` identifies the failed operation, while `readiness.blockers` records only a closed set of capability labels without emitting provider text, and the two round-trip booleans plus `containsSecrets=false` remain explicit.
- Readiness now distinguishes the profile states `Loaded`, `NotLoaded`, `Unknown` and `QueryUnavailable`; an unrecognized state or readiness label fails closed. The artifact privacy guard validates both readiness contracts in the current exit/target evidence, and the handoff guard requires that coverage before reporting `Pass`.
- Phase-4 runners now disable MSBuild node reuse for their bounded invocation and keep process ownership explicit, reducing cross-run contamination risk; cleanup remains scoped to newly owned windowless processes and uses a bounded post-run drain (up to 8 attempts at 250 ms) to catch delayed child hosts.
- The exit and target-certification runners invoke the same drain after each bounded child probe and once more before writing their aggregate artifact, so a delayed host cannot interfere with the next probe while cleanup counters remain unique and structural.
...
- The six prior WP4.1/WP4.2/WP4.3/WP4.4/WP4.6/WP4.7 gate runners were re-run against the current suite and each reports `181` tests with `Pass`; this is recorded as the prior-gate regression summary without changing their historical work-package scopes.
