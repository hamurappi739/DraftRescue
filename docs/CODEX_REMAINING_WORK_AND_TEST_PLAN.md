...
- bounded checkpoint scheduling, explicit runtime composition, and background execution with explicit trailing-debounce/max-dirty-age policy, coalescing, capacity bound, delayed retry, signal/due-time wakeup, and protect-before-repository delegation — implemented and tested;
